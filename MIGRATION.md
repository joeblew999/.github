...
**Per-version detail lives in [CHANGELOG.md](./CHANGELOG.md)** (generated by
git-cliff — never hand-edited). This file lists ONLY the **breaking** versions
that renamed files/tasks. Everything else is additive.
...
3. `mise run mise:repo:bootstrap` → `mise run ci` locally → green → push →
   confirm the matrix is green.

That's the whole upgrade for an **additive** bump. If your jump **crosses a
breaking version listed below**, do that version's rename steps first. Re-running
the bootstrap is always safe + idempotent — it brings the repo to exactly the
...
**1. Bump every `?ref=`** in `mise.toml` `[task_config].includes` → the newest tag
(anything `≥ v0.40.0` has this layout).
